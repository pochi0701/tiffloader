...
void bmp_image::assign_data (const char* data, FMT format)
{
	if (format == FMT::FMT_RGBA) {
		int ptr = 0;
	...
	else if (format == FMT::FMT_GREYALPHA) {
		int ptr = 0;
		for (auto i = 0;i < nHeight;i++) {
			for (auto j = 0;j < nWidth;j++) {
				rgbImage[(nHeight - i - 1) * nWidth + j].rgbBlue = data[ptr * 2 + 0];
				rgbImage[(nHeight - i - 1) * nWidth + j].rgbGreen = data[ptr * 2 + 0];
				rgbImage[(nHeight - i - 1) * nWidth + j].rgbRed = data[ptr * 2 + 0];
				ptr++;
			}
		}
	}
